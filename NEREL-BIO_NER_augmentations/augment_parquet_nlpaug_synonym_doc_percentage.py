...
AUGMENT_PERCENT = 0.5           # 10% of docs per file
NUM_AUG_PER_DOC = 1             # Number of augmentations per doc
AUGMENTATION_TYPE = "synonym"  # or "backtranslation", "insertion", etc.
AUGMENT_PERCENT_DATASET = 0.1

# === Output Directories (dynamic based on augmentation type and percent) ===
...
def augment_file(input_path, lang_dir):
    ...
    num_to_sample = int(len(all_doc_ids) * AUGMENT_PERCENT_DATASET)
    sampled_doc_ids = set(random.sample(list(all_doc_ids), num_to_sample))
