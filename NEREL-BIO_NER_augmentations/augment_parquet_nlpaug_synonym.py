...
AUGMENT_PERCENT = 0.5            # 50% of words per doc
NUM_AUG_PER_DOC = 2              # Number of augmentations per doc
AUGMENTATION_TYPE = "synonym"    # "synonym", "insertion", etc.

# === Output Directories (reflecting full doc augmentation) ===
AUG_STR = f"pdoc200_pword{int(AUGMENT_PERCENT * 100)}"
...
def augment_file(input_path, lang_dir):
    ...
    for doc_id, group in tqdm(grouped, desc=f"→ {os.path.basename(input_path)}"):
        ...
        base_txt_path = os.path.join(OUTPUT_TEXT_DIR, lang_dir, split, f"{doc_id}.txt")
        ...
        for i in range(NUM_AUG_PER_DOC):
            ...
            new_doc_id = f"{doc_id}_aug_{AUGMENTATION_TYPE}_pdoc200_pword{int(AUGMENT_PERCENT * 100)}_{i+1}"
    ...
    percent_suffix = f"_augmented_{AUGMENTATION_TYPE}_pdoc200_pword{int(AUGMENT_PERCENT * 100)}"
