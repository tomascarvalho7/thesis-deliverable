...
def calculate_retrieval_metrics(pred_df: pd.DataFrame, sample_pk2true_cui: Dict[int, str]):
    ...
    return {
        "Acc@1": acc_1_sum / num_samples,
        "Acc@5": acc_5_sum / num_samples,
...
def filter_vocab(vocab):
    en_cuis_set = set(vocab[vocab["lang"] == "ENG"]["CUI"].unique())
    ru_cuis_set = set(vocab[vocab["lang"] == "RUS"]["CUI"].unique())
    merged_cuis = en_cuis_set.union(ru_cuis_set)
    vocab = vocab[vocab["CUI"].isin(merged_cuis)][["CUI", "semantic_type", "concept_name"]]
    print(f"Created bilingual vocab: {vocab.shape}")
    return vocab
...
# ===================== Training Loop (Bilingual) =====================

def run_training(epochs=3):
    # ---- Load LOCAL parquet datasets ----
    ru_data_train = pd.read_parquet("./data/augmented_backtranslation_entity_linking_full/ru/bionnel_ru_train.parquet")
    ru_data_dev = pd.read_parquet("./data/augmented_backtranslation_entity_linking_full/ru/bionnel_ru_dev.parquet")
    en_data_train = pd.read_parquet("./data/augmented_backtranslation_entity_linking_full/en/bionnel_en_train.parquet")
    en_data_dev = pd.read_parquet("./data/augmented_backtranslation_entity_linking_full/en/bionnel_en_dev.parquet")
    vocab = pd.read_parquet("./data/vocabular/bionnel_vocab_bilingual.parquet")

    # Merge bilingual sets
    train_data = pd.concat([en_data_train, ru_data_train], ignore_index=True)
    dev_data = pd.concat([en_data_dev, ru_data_dev], ignore_index=True)

    bi_vocab = filter_vocab(vocab)
    ...
    for epoch in range(1, epochs + 1):
        print(f"\n===== Epoch {epoch} (Bilingual) =====")

        predictions_df = make_predictions(
            entities_df=dev_data,
            tokenizer=tokenizer,
            bert_encoder=bert_encoder,
            vocab=bi_vocab,
        ...
        dev_data[SAMPLE_PK_COL] = dev_data.apply(create_row_primary_key, axis=1)

        merged_df = dev_data.merge(
